"use client"
import { Users, Play, Pause, SkipForward, Trash2, Clock, Music, Star } from "lucide-react"
...
export default function ParticipantManagement() {
  const { participants, deleteParticipant, updateParticipantStatus } = useKaraokeData()
  ...
  const finishedParticipants = participants.filter((p) => p.status === "Terminé")
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white">Gestion des Participants</h1>
            <p className="text-purple-200">Gérez la file d'attente et les performances</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <Card className="bg-gradient-to-r from-green-500/20 to-emerald-600/20 backdrop-blur-sm border-green-500/30">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-2xl font-bold text-white">{currentParticipant ? 1 : 0}</p>
          ...
          <Card className="bg-gradient-to-r from-yellow-500/20 to-orange-600/20 backdrop-blur-sm border-yellow-500/30">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-2xl font-bold text-white">{waitingParticipants.length}</p>
          ...
          <Card className="bg-gradient-to-r from-gray-500/20 to-slate-600/20 backdrop-blur-sm border-gray-500/30">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-2xl font-bold text-white">{finishedParticipants.length}</p>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-4 mt-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-white">{currentParticipant.song?.title}</span>
                    </div>
                    <span className="text-green-300">•</span>
                    <span className="text-green-200">{currentParticipant.song?.artist}</span>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  <Button
                    ...
                    onClick={() => {
                      if (currentParticipant.song?.youtubeId) {
                        window.open(`https://www.youtube.com/watch?v=${currentParticipant.song.youtubeId}`, "_blank")
                      }
                    }}
                  >
                  ...
                  <Button
                    onClick={() => handleStatusChange(currentParticipant.id, "Terminé")}
        ...
        <Card className="bg-black/20 backdrop-blur-sm border-white/10">
          <CardHeader>
            ...
            <CardDescription className="text-purple-200">Gérez l'ordre de passage et les statuts</CardDescription>
          </CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-white/10">
                    ...
                    <TableHead className="text-purple-200">Inscription</TableHead>
                ...
                <TableBody>
                  {participants.map((participant) => (
                    <TableRow key={participant.id} className="border-white/10 hover:bg-white/5">
                      <TableCell>
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            {participant.email && <p className="text-purple-200 text-sm">{participant.email}</p>}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div>
                          <p className="text-white font-medium">{participant.song?.title}</p>
                          <p className="text-purple-200 text-sm">{participant.song?.artist}</p>
                      ...
                      <TableCell>
                        <p className="text-purple-200 text-sm">{new Date(participant.registeredAt).toLocaleString()}</p>
                      </TableCell>
                      <TableCell>
                        <div className="flex space-x-2">
                          {participant.status === "En attente" && (
                            <Button
                              ...
                              size="sm"
                              className="border-green-500/30 text-green-300 hover:bg-green-500/20 hover:text-white"
                              onClick={() => handleStatusChange(participant.id, "En cours")}
                            >
        ...
        {participants.length === 0 && (
          <Card className="bg-black/20 backdrop-blur-sm border-white/10 mt-6">
            <CardContent className="p-8 text-center">
              ...
              <h3 className="text-white text-lg font-semibold mb-2">Aucun participant inscrit</h3>
              <p className="text-purple-200 mb-4">
                Les participants peuvent s'inscrire via le formulaire d'inscription.
              </p>
              <Button
