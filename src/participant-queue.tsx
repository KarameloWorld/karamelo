...
import { Clock, Music, Users, Mic, Play, Star, ArrowLeft, RefreshCw } from "lucide-react"
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white">File d'attente Karaoké</h1>
            <p className="text-purple-200">Suivez votre position en temps réel</p>
          </div>
          ...
          <CardContent>
            <div className="flex items-center space-x-4">
              ...
              <div className="text-right">
                ...
                <Progress value={(currentSong.currentTime / currentSong.duration) * 100} className="w-24 h-2 mt-1" />
                <p className="text-purple-200 text-xs mt-1">
                  {Math.floor(currentSong.currentTime / 60)}:
                  {(currentSong.currentTime % 60).toString().padStart(2, "0")} /{Math.floor(currentSong.duration / 60)}:
        ...
        <Card className="bg-black/20 backdrop-blur-sm border-white/10">
          <CardHeader>
            ...
            <CardDescription className="text-purple-200">Ordre de passage et temps d'attente estimé</CardDescription>
          </CardHeader>
          <CardContent>
            {queue.length === 0 ? (
              <div className="text-center py-8">
                <Users className="h-12 w-12 text-purple-400 mx-auto mb-4" />
                <h3 className="text-white text-lg font-semibold mb-2">Aucun participant en attente</h3>
                <p className="text-purple-200">La file d'attente est vide pour le moment.</p>
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-2">
                          <p className="text-purple-200 text-sm">{participant.song?.title}</p>
                          <span className="text-purple-300">•</span>
                          <p className="text-purple-300 text-sm">{participant.song?.artist}</p>
                        </div>
                      ...
                      <div className="text-right">
                        {isNext ? (
                          <Badge className="bg-green-500/20 text-green-300 border-green-500/30">À vous bientôt !</Badge>
                        ) : (
                        ...
                        <p className="text-purple-200 text-xs mt-1">
                          {new Date(participant.registeredAt).toLocaleTimeString()}
                        </p>
                      </div>
